...
export function GoalsAnalytics({ goals }: GoalsAnalyticsProps) {
  ...
  const progressRanges = {
    ...
    "Early Progress (1-25%)": goals.filter(
      (g) => g.progress > 0 && g.progress <= 25
    ).length,
    "In Progress (26-75%)": goals.filter(
      (g) => g.progress > 25 && g.progress <= 75
    ).length,
    "Nearly Complete (76-99%)": goals.filter(
